...
app.post('/api/settings', (req, res) => {
    const { ALERT_EMAIL_TO, TEAMS_WEBHOOK_URL, TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID } = req.body;
    
    if (ALERT_EMAIL_TO !== undefined) process.env.ALERT_EMAIL_TO = ALERT_EMAIL_TO;
    if (TEAMS_WEBHOOK_URL !== undefined) process.env.TEAMS_WEBHOOK_URL = TEAMS_WEBHOOK_URL;
    ...
    try {
        ...
        if (ALERT_EMAIL_TO !== undefined) updateEnvVar('ALERT_EMAIL_TO', ALERT_EMAIL_TO);
        if (TEAMS_WEBHOOK_URL !== undefined) updateEnvVar('TEAMS_WEBHOOK_URL', TEAMS_WEBHOOK_URL);
        if (TELEGRAM_BOT_TOKEN !== undefined) updateEnvVar('TELEGRAM_BOT_TOKEN', TELEGRAM_BOT_TOKEN);
        if (TELEGRAM_CHAT_ID !== undefined) updateEnvVar('TELEGRAM_CHAT_ID', TELEGRAM_CHAT_ID);

        // Remove empty lines if any double newlines exist
        envContent = envContent.replace(/\n\n+/g, '\n').trim() + '\n';
        
        fs.writeFileSync(envPath, envContent, 'utf8');
        res.json({ success: true, message: 'Configurações salvas com sucesso' });
    } catch (err) {
        console.error('Error saving settings to .env', err);
